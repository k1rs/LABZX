...
void WriteTextFile(struct BigSize Number, const char* file)
{
	...
	for (i = DecNumber.Count - 1; i >= 1; i--) fprintf(out, "%d", DecNumber.Number[i]);
	fprintf(out, "%d", DecNumber.Number[0]);
	fclose(out);
}
...
void PrintBigSize(struct BigSize Number) //Вывод числа
{
	...
	for (i = Number.Count - 1; i >= 1; i--) printf("%d", Number.Number[i]);
...
{
	struct BigSize res, Number;
	res = Allocate(res, 1);
	Number = Allocate(Number, 1);
	Number.Count = strlen(buffer);
...
char* ToString(struct BigSize number)
{
	...
	//printf("qwer");
	res = RemoveBinTen(number);
	for (i=res.Count-1;i>0;i--) printf("%d",res.Number[i]);
	printf("%d",res.Number[0]);
	for (i = 0; i < 256; i++) str[i] = 0;

	for (i = res.Count - 1; i > -1; i--){ str[0] = res.Number[i]+'0'; printf("%d",res.Number[i]); j++; }
